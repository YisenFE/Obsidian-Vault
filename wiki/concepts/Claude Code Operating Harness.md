...
Claude Code Operating Harness（Claude Code 操作支架）是用户围绕 Claude Code 配置的一整套日常运行环境：skills/commands、hooks、subagents、rules/memory、MCPs、plugins、editor、worktree/tmux、statusline、CI review 和 dynamic workflows 等共同构成可复用、可观察、可约束的工作流。

## 为什么重要

Claude Code 本身只是 agent 产品；真正决定高吞吐和稳定性的，是用户如何把重复流程、上下文入口、权限边界、自动检查、并行隔离和编辑器观察面组合起来。Claude Code shorthand 来源把 user harness 从抽象 guides/sensors 落到 `~/.claude` 配置和日常命令层；dynamic workflows 来源则把这层能力推进到“按任务临时生成 harness”。
...
- Dynamic workflows 让 Claude Code 现场生成 JavaScript orchestration，按任务 spawn subagents、选择模型、选择 worktree 隔离，并在中断后恢复。
- Dynamic workflows 更适合长时程、大规模并行、高结构化或对抗性任务；常规 coding task 不一定需要额外 workflow compute。
...
- Dynamic workflows 应与 skills、commands 和 subagents 如何分层，才能既可复用又不过度工程化？
