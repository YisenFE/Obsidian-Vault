...
- Claude Code dynamic workflows 来源把 evaluator 进一步模式化为 adversarial verification：每个 spawned agent 的输出可由另一个独立 spawned agent 按 rubric 检查。
- Tournament 和 generate-and-filter 模式也依赖 judge/evaluator agent：先生成多个候选，再用比较、去重、rubric 或 pairwise judging 选择高质量结果。
...
- tournament judge 的比较顺序、rubric 和模型选择会不会显著影响最终赢家？
