...
Anthropic 的长时程智能体来源展示了一条演化线：先解决跨会话连续性（session continuity），再解决多智能体质量控制，最后把多智能体 harness 变成 Claude Code 内可按任务动态生成的 workflow。早期 harness 用 initializer agent、coding agent、feature list、`claude-progress.txt`、git history 和 `init.sh` 处理“新上下文没有记忆”的问题；核心目标是可靠交接：每轮先理解当前状态，验证基础应用仍可运行，再实现一个增量，并留下干净状态。
...
长时程 harness 不是固定 recipe。它通常从外化记忆和任务状态开始，只在当前模型确实失败的地方增加专门角色。模型能力提升后，部分脚手架会变成开销；但可靠基线向外扩展后，团队也能尝试更复杂、更长期的 harness。Dynamic workflows 的启发是：harness 不必总是预先设计成静态系统，也可以在任务启动时由模型根据目标、验证难度和资源预算生成。
